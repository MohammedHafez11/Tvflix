...
const heroBanners = function({results: movieList}){
    ...
    for(const [index, movie] of movieList.entries()){
        ...
        <img src="${imageBaseURL}w1280${backdrop_path}" alt="${title}" class="img-cover" loading=${index === 0 ? "eager" : "lazy"}>
                ...
                <p class="genere">${genreList.asString(genre_ids)}</p>

                <p class="banner-text">${overview}</p>
                <a href="detail.html" class="btn" onclick="getMovieDetail(${id})">
                <img src="assets/images/play_circle.png" aria-hidden="true" width="24" height="24" alt="" srcset="">
    ...
    for(const {title, path} of homePageSection){
        fetchDateFromServer(`https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1`, createMovieList, title)
    }
}
